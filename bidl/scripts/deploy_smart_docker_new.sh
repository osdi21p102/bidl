...
while read line; do
    ...
    echo ${node} ${host}
    ssh -n anonymous@${host} "cd ~; rm -rf logs; mkdir logs; docker run --name smart${node} --net=host --cap-add NET_ADMIN smart bash /home/runscripts/smartrun.sh bftsmart.demo.microbenchmarks.ThroughputLatencyServer ${node} 10 0 0 false nosig rwd >logs/log_${node}.log 2>&1 &"
