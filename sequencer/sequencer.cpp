 ...
 *      3. Insert sequence number(4 bytes) in front of the packet body.
 *      4. Broadcast the processed packet to 9999 port.
 * 
 */
#include <stdio.h>
#include <sys/types.h>
...
int main() {
    ...
    while(1) {
        ...
        *ptr = seq++;
        ...
        if(tot % temp == 0) {
            auto t2 = Clock::now();
            auto interval = std::chrono::duration_cast<std::chrono::milliseconds>(t2-t1).count() / 1000.0;
            t1 = t2;
            std::cout << interval  << " tps:" << temp / interval << '\n';
